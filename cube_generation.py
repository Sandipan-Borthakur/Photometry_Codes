...
#    model = deepCR(mask='ACS-WFC-F606W-2-32', inpaint='ACS-WFC-F606W-2-32', device='CPU')
    ...
    for filename in filelist:
        ...
        t_exposure_begin = (t.jd)  # to convert from IST to UT
...
def gen_flat(data_path, masterBias):
    ...
    f_cube=[]
    for i in range(flatCube.shape[0]):
        f_cube.append(flatCube[i,:,:])
    flatCube=np.array(f_cube)
...
def gen_img_cube(data_path, sciencelist, masterBias, masterFlat):
    ...
    s=scienceCube.shape
    masterBias = masterBias[:s[1],:s[2]]
    masterFlat = masterFlat[:s[1],:s[2]]
    print(s,masterFlat.shape,masterBias.shape)
...
def cube_gen(data_path, onlyfile_spec_filter):
    
    masterBias = gen_bias(data_path)

    masterFlat = gen_flat(data_path, masterBias)
